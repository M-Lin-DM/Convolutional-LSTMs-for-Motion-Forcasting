...
ds_test = tf.data.Dataset.from_generator(
    tc_inputs_generator,
    args=[tc_start_frame, tc_end_frame, test_dir],
...
# ds_test = ds_test.take(steps_per_epoch_tuning)

# PREDICT USING MODEL
Y_pred = model.predict(ds_test)
np.save(r'D:\Datasets\Fish\processed\targets\test\Y_pred', Y_pred)

Y_tilde = np.load(r'D:\Datasets\Fish\processed\targets\test\Y_tilde.npy')
Y_pred = np.load(r'D:\Datasets\Fish\processed\targets\test\Y_pred.npy')
...
# NOTE: y_pred must have the same length as crop_to_valid_range(Y_label, n, h) to perfrom ROC analysis. The reason is drop_remainder was set to False during batching
...
fpr, tpr, thresholds = roc_curve(crop_to_valid_range(Y_label, n, h)[0:len(Y_pred)+0], Y_pred[:, 0], drop_intermediate=True)
auc = roc_auc_score(crop_to_valid_range(Y_label, n, h)[0:len(Y_pred)+0], Y_pred[:, 0])
print(f"auc: {auc}")
